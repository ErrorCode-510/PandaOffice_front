...
## 🧾 급여관리
### 🛠 기능설명
- 급여자료입력: 직원에게 지급하는 급여 및 4대보험과 같은 공제금액을 조회하고 입력하는 관리 페이지.<br>
귀속연월은 당월을 기준으로 자동선택되며 급여, 상여, 급여+상여로 구분한다. (지급일은 귀속연월의 25일로 고정됨)<br>
직원 테이블에 원하는 직원을 선택하면 해당 직원의 지급항목과 공제항목이 산출식에 자동계산되어 출력되고 재계산 혹은 수기로 수정할 수 있다.<br>
지급항목 테이블에서 각 지급항목의 과세여부와 총 지급액을 표시하고 공제항목 테이블에선 공제항목의 총액과 차인지급액을 계산하여 표시한다.

- 급여조회: 로그인한 직원의 한달 급여기록을 조회하는 페이지로 해당 직원의 부서, 직급, 급여목록, 은행 정보 등을 확인할 수 있다.<br>
