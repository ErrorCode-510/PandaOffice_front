...
export const { notice: { getNotice, setNotice, getNoticeByCategory } } = createActions({
    ...
    [GET_NOTICE_BY_CATEGORY]: result => ({ noticesByCategory: result.data })
});
...
const noticeReducer = handleActions({
    ...
    [GET_NOTICE_BY_CATEGORY]: (state, { payload }) => ({ ...state, noticesByCategory: payload.noticesByCategory })
}, initialState);
